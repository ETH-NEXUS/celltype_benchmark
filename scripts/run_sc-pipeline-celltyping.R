...
all_celltypes_full_ct_name = unique(all_celltypes_full_ct_name)
metadata(sce_data) = c(metadata(sce_data), list(all_celltypes_full_ct_name=all_celltypes_full_ct_name))
metadata(sce_data) = c(metadata(sce_data), list(all_celltypes=gsub(pattern = "([^_]+)_.*", "\\1", all_celltypes_full_ct_name)))
...
metadata(sce_data) = c(metadata(sce_data), list(all_major_types=gsub(pattern = "([^_]+)_.*", "\\1", major_types)))
...
all.major.types = c(metadata(sce_data)$all_major_types, "uncertain", "unknown")
...
for(ii in seq(length(minor_types))){
...
all.cell.types = c(metadata(sce_data)$all_celltypes, "uncertain", "unknown")
...
all.cell.types.full <- c(metadata(sce_data)$all_celltypes_full_ct_name, "uncertain", "unknown")
...
tab.ci = binconf(tab[nrow(tab)-1, seq(ncol(tab)-1)], rep(tab[nrow(tab)-1, ncol(tab)], ncol(tab)-1), method="wilson")*100
