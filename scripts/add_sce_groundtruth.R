...
option_list = list(
  ...
  make_option("--matched_labels", type = "character", help = "Path to dictionary file mapping sample-specific cell type labels to universal labels for cross-sample comparisons."),
...
add_sce_groundtruth <- function(rds_path, labels_path, matched_labels_dictionary = NULL, output_path){
...
add_sce_groundtruth(opt$sce_in, opt$labels_in, opt$matched_labels, opt$sce_out)
